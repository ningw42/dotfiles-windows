...
`~/.codex/{AGENTS,RTK}.md`, `~/.copilot/copilot-instructions.md`, and the copilot rtk hook. They are
(re)generated by `.chezmoiscripts/run_onchange_rtk-init.ps1.tmpl` (`rtk init -g --codex` / `--copilot`),
only when the rtk binary version changes. **Claude Code is hook-only:** the transparent `rtk hook claude`
PreToolUse hook (wired in `dot_claude/settings.json.tmpl`, which stays chezmoi-owned) makes the RTK.md /
CLAUDE.md prompt injection redundant, so we don't run `rtk init` for Claude and the orphaned
`~/.claude/{RTK,CLAUDE}.md` are purged via `.chezmoiremove` (mirrors nixfiles' hook-only claude-code module).
